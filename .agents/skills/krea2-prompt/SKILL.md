...
`instruct` — deterministic faithful-expansion synthesis from an explicit procedure.
...
- **Positive prompting.** Do not append a negative prompt. Translate avoidances
  to the desired visible state where possible (`no blur` → `tack-sharp focus`;
  `no people` → `empty, unoccupied space`). Preserve an explicit absence whose
...
- Sparse input: enrich with useful visual information — grounded subject
...
- Keep attributes close to the subjects they modify. For multi-subject scenes,
  ...
  seated elderly man across a narrow café table.` over a disconnected list of
  keywords.
