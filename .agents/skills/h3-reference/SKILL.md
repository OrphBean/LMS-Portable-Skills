---
name: h3-reference
description: Rewrite a request plus one or more reference assets into MiniMax H3 full-reference (Ref2VA) format with reference labels, retention analysis, and a detailed description. Use when supplied assets provide guidance (identity, appearance, composition, camera, motion, trajectory, performance, style, temporal structure, storyboard planning, audio) without serving as literal boundary frames. Do NOT use when an image is only the opening frame (h3-i2v), an endpoint pair (h3-first-last), or the final frame (h3-last-frame).
---

# H3 reference-mode prompt
...
`instruct` — reference-role assignment and label-stable rewrite from an explicit procedure.

## Inputs / semantic interpretation — reference roles
...
- **audio reuse** — the source audio signal itself is reused in full or in
...
- **audio reference** — the source signal is not copied; its audible
...
occurring physically in the scene. Do not place ordinary sound effects —
footsteps, impacts, doors, machinery, explosions, glass breaking, animal
sounds, environmental effects — under `non_diegetic_music`. They belong to the
...
## Output format — six sections, in order
...
- `<Subject N>`: reusable visible content — a person, animal, object, scene,
...
- `<Video N>`: whole-video relationships — editing source, continuation, or
  camera/cuts/rhythm/temporal structure. Visible content reused from a video
  still belongs under `<Subject N>`.
- `<Audio N>`: an audio asset or enabled synchronized track — copied signal,
...
appearance and wherever its role applies. Aim for 350–500 words for generation
...
Ambience and physical sound across the whole video, 1–4 sentences in one
...
labels at first appearance; motion follows the discipline (cause → effect, no
