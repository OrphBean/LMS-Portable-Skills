# h3-i2v
Use when Build a MiniMax H3 image-to-video (I2VA) prompt where the supplied image is the literal opening frame. Use when a supplied image is the authoritative starting visual state of the video. Do NOT use when the image defines only the ending (h3-last-frame), both endpoints (h3-first-last), or is guidance-only (h3-reference).
...
`instruct` â€” deterministic develop-forward construction from an explicit procedure.
...
- Follow the structure: first-frame anchor â†’ action onset â†’ continuous
  development â†’ result or reaction.
...
- If the image is not meant to be the opening frame, this is not i2v â€” route to
