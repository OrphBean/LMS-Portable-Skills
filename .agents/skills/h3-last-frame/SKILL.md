# h3-last-frame
Use when Build a MiniMax H3 final-frame-conditioned (L2VA) prompt that converges onto a supplied final image. Use when a supplied image defines the required ending visual state. Do NOT use when the image is the opening frame (h3-i2v), an endpoint pair (h3-first-last), or guidance-only (h3-reference).
...
`instruct` â€” deterministic converge-forward construction from an explicit procedure.
...
- The alignment instruction must be the first line of the prompt body
  ...
  `How the reference pictures align with the target video â€” <Picture 1> (from [Shot N]) aligns with the S.SS-second mark of the target video.`
...
- The image belongs to the END of the sequence. Never describe it as the
  opening frame.
- Follow the structure: plausible preceding state â†’ explicit action and
  transition path â†’ gradual convergence in the final shot â†’ last-frame landing.
