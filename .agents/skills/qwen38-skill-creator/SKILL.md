---
name: qwen38-skill-creator
description: >
  Compile source knowledge, docs, workflows, and existing frontier-model
  skills into compact runtime skills for Qwen 3.8 27B. Activate when creating
  a new Qwen-native skill, converting OpenAI/DeepSeek/Claude/Gemma/other
  skills for Qwen 3.8, optimising an existing Qwen skill, or asked to "make a
  Qwen skill", "compile this into a skill", or "convert this guide for Qwen".
  Runs an 11-pass compilation optimised for reliability per runtime token
  while defeating Qwen's default overthinking mode. Handles dual-mode
  reasoning (thinking vs instruct), XML tool-calling, Jinja chat-template
  constraints, vision/video sampling, downstream-model orientation for MiniMax
  H3 (video), FLUX Klein 9B (image), and Krea 2 (image), and a screenwriter's
  discipline (in-frame only; nouns and verbs; cause-and-effect; motion logic;
  screen direction; photographic realism). Do NOT use for general Qwen
  prompting guides, one-shot prompts, skills targeting another runtime model,
  or document summarisation.
compatibility: "Design-time: any agent. Output targets Qwen 3.8 27B served locally (e.g. OpenCode); see references/target-model-profile.md."
---
...
Optimise `rich source knowledge → frontier-model analysis → compact Qwen
...
Qwen 3.8 27B is a dense 27.78-billion-parameter multimodal model — all
...
video ingestion with fps / max_frames / pixel-budget sampling; user-intent →
...
### Rule A — Always declare the reasoning mode
...
(medium/xhigh). Default to `instruct`. Never leave it unspecified — the serving
default `xhigh` inherits the overthinking trap. Use `low` / `medium` / `xhigh`;
`high` is aliased to `xhigh`, so do not rely on it.

### Rule B — Respect the Jinja template contract
...
### Rule C — Choose the tool-calling path explicitly
...
### Rule D — Budget reasoning context
...
### Rule E — Tune vision/video sampling to the content
...
### Rule F — Use RAG instead of parametric trivia
...
Compiled skills write prompts for visual generation — MiniMax H3 (video),
...
- **Screen direction.** Keep screen sides, direction of travel, and eye-lines
  ...
  reads as a reversal — write it only when meant.
...
Qwen's own reasoning tokens also consume the context window — a skill forcing
...
- JSON WITH tools: do NOT combine with strict JSON Schema decoding — the model
...
- **MiniMax H3** — video. The prompt is a temporal timeline: what happens, in
...
- **FLUX Klein 9B** — image. Natural-language prose; no negative prompt
  (translate avoidances to the desired visible state); front-load subject and
  action; 30–80 words; lighting high-impact; photographic language only when
  it serves visible intent; no prompt upsampling.
- **Krea 2** — image. One natural-language paragraph; faithful expansion that
...
8. **Pass 8 — Define output semantics.** Specify fields, formats, contracts.
...
9. **Pass 9 — Anticipate failure modes.** Identify likely Qwen failures
...
10. **Pass 10 — Add validation.** Create a concise self-check focused on
    high-risk errors, not a bloated checklist.
11. **Pass 11 — Lint.** Inspect for ambiguous pronouns; compound instructions;
...
`high` → `xhigh` alias); single system message at position zero; functioning
...
embed it only when genuinely required — the reasoning-mode declaration
...
### Example 1 — new skill from a verbose motion-prompt design doc
...
the same screen side across cuts. Never write mood — write the physical
...
> - Every action states its visible consequence (cause → effect).
...
### Example 2 — frontier-skill conversion
...
- [ ] instils the screenwriter's frame — only what is in the frame, nouns and verbs, no abstract-mood prose; cause-and-effect and motion continuity; screen direction; real-human photographic realism;
