...
`instruct` â€” pure routing. No reasoning trace needed.
...
- **t2i** â€” no supplied image is modified or semantically combined with the
  output. Images inspected only for inspiration do not make the request
  image-to-image.
- **edit** â€” exactly one source image is passed to Klein and the user wants to
  change, replace, add, remove, restyle, relight, modify, or preserve-and-vary
  that source.
- **multiref** â€” two or more images are passed to Klein and the output must
...
- One image with a change instruction â†’ `edit`.
- One image described only as inspiration or style ("a scene like this") with no
  intent to modify it â†’ `t2i`.
- Two or more images that each contribute distinct attributes â†’ `multiref`.
...
freedom never overrides items 1â€“3.
