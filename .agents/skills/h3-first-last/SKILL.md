# h3-first-last
Use when Build a MiniMax H3 first-frame + last-frame (FL2VA) prompt that transitions between a required opening image and a required final image. Use when two images define the required beginning and ending states. Do NOT use for a single opening frame (h3-i2v), a single final frame (h3-last-frame), or guidance-only references (h3-reference).
...
`instruct` â€” deterministic interpolation-path construction from an explicit procedure.
...
- The alignment instruction must be the first line of the prompt body
  ...
  `How the reference pictures align with the target video â€” <Picture 1> (from [Shot 1]) aligns with the 0.00-second mark of the target video; <Picture 2> (from [Shot N]) aligns with the S.SS-second mark of the target video.`
...
- The body must describe the MOTION PATH between the endpoints, not two static
  image descriptions.
- Follow the structure: first-frame state â†’ observable intermediate changes â†’
  progressively narrowing differences â†’ last-frame state.
