...
`instruct` â€” deterministic extraction and condensation from an explicit procedure. No reasoning trace needed.
...
- **Priority rules.** The user's hierarchy when constraints conflict (for
  example: user facts â†’ reference preservation â†’ format â†’ style overlays â†’
...
- Prompt outputs already captured â€” record only their decisions and conventions,
  not their full text.
...
7. Assemble the document in the output format below. Keep it as compact as
   correctness allows â€” the goal is the smallest document a fresh chat can
...
```
COMPACTION â€” <session label>

INTENT
<one or two sentences: what the session is producing>

REFERENCES
R1 <role: identity/appearance/literal-frame/...> â€” <dense verbal description>
R2 <role> â€” <dense verbal description>
