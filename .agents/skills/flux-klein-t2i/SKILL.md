---
name: flux-klein-t2i
description: Build a FLUX.2 [klein] text-to-image prompt from a textual request. Use when no supplied image is modified or semantically combined with the output. Do NOT use for single-reference editing (flux-klein-edit) or multi-reference composition (flux-klein-multiref).
---

# FLUX.2 Klein T2I prompt
...
`instruct` — deterministic prose synthesis from an explicit procedure.
...
discretionary embellishment. Creative freedom never overrides items 1–2.

## Creative freedom

You choose secondary visual detail, natural movement, atmospheric detail, and
cinematic phrasing — within the user's facts, the requested subject, and the
